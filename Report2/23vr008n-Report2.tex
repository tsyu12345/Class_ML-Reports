...
\section{第7回授業 : 問題２}
    ...
    \begin{flalign*}
        & E(\hat{theta}) = -\sum_{n=1}^{N}\left\{y_n\log\hat{y}(x_n) + (1-y_n)\log(1-\hat{y}(x_n))\right\}
    \end{flalign*}
    ...
    具体的には、
    \begin{flalign*}
        & \hat{y}(x_n) = y_n = 0 \text{のとき} \\
        & \hat{y}(x_n) = y_n = 1 \text{のとき}
    \end{flalign*}
    ということになる。これをクロスエントロピー誤差関数の式に代入すると、$y_n = 0$の場合は
    \begin{flalign*}
        & E(\hat{\theta}) = -\sum_{n=1}^{N}\left\{y_n\log\hat{y}(x_n) + (1-y_n)\log(1-\hat{y}(x_n))\right\} \\
        & = -\sum_{n=1}^{N}\left\{0\log\hat{y}(x_n) + (1-0)\log(1-\hat{y}(x_n))\right\} \\
        & = -\sum_{n=1}^{N}\left\{\log(1-\hat{y}(x_n))\right\} \\
    ...
    \begin{flalign*}
        & E(\hat{\theta}) = -\sum_{n=1}^{N}\left\{y_n\log\hat{y}(x_n) + (1-y_n)\log(1-\hat{y}(x_n))\right\} \\
        & = -\sum_{n=1}^{N}\left\{1\log\hat{y}(x_n) + (1-1)\log(1-\hat{y}(x_n))\right\} \\
        & = -\sum_{n=1}^{N}\left\{\log\hat{y}(x_n)\right\} \\
        & = -\sum_{n=1}^{N}\left\{\log1\right\} \\
        & = 0
    \end{flalign*}
    となる。\par 
